...
if (isset($_GET['deleteId']) && !empty($_GET['deleteId'])) {
    ...
    $dataObj->deleteGradesOrchid($deleteId, $viewGrades);
} 



$insert = $dataObj->getSingleDataOrchid();
...
<body class="bg-gray-200 font-sans">
    ...
    <div class=" max-w-[750px] mx-auto bg-amber-50 p-6 rounded-md shadow-md mt-8">
        ...
        <?php 
            ...
            $queryStudentDetails = "SELECT t1.ID, t1.Fullname, t1.Grade_level, t1.Teacher, t1.Section FROM studentdata2 t1 LEFT JOIN grading_system_2 t2 ON t1.ID = t2.student_id WHERE t1.ID = $personalId";
            ...
            <table class="w-full border-collapse  mb-2">
                ...
                <tbody>
                    <?php
                        ...
                        FROM grading_system_2 t1 
                        LEFT JOIN studentdata2 t2 ON t1.student_id = t2.ID 
                    ...
                    <tr>
                        ...
                        <?php
                                ...
                                $editLink = "../crud/update_grades_2.php?updateId={$row['ID']}";
                                ...
                                if ($isAdmin) {
                                   ...
                                   // If the user is not an administrator, check if the status is pending or denied
                                        if ($isStatusPending) {
                                            ...
                                            // If the status is approved,  disable the edit button   
                                ...
                                ?>
                            <a href="grades_2.php?deleteId=<?php echo $row['ID'];?>&viewGrades=<?php echo $studentId;?>" class="delete cursor-pointer ml-2 lg:ml-0">
        ...
        <?php
            if ($isAdmin) {
                echo '<a href="../crud/create_grades_2.php?insertId=' . $insert['ID'] . '" class="hidden bg-blue-500 text-white mx-2 py-2 px-4 rounded-md hover:bg-blue-600 mb-2 md:mb-0">Insert</a>';
            }else{
                echo '<a href="../crud/create_grades_2.php?insertId=' . $insert['ID'] . '" class="bg-blue-500 text-white mx-2 py-2 px-4 rounded-md hover:bg-blue-600 mb-2 md:mb-0">Insert</a>';
            }
        ?>
        
        <?php
            if ($isAdmin) {
                // Assuming $insert is an associative array containing student details
                if (!empty($insert) && isset($insert['student_id'])) {
                    // Check if there are pending grades for approval
                    $pendingQuery = "SELECT COUNT(*) as pendingCount FROM grading_system_2 WHERE student_id = {$insert['student_id']} AND status = 'pending'";
                    ...
                    if ($pendingResult) {
                        ...
                        if ($pendingCount > 0) {
                            echo '<a href="orchid_approval.php?approvalId=' . $insert['student_id'] . '" class="bg-purple-500 text-white mx-2 py-2 px-4 rounded-md hover:bg-purple-600 mb-2 md:mb-0">Approval</a>';
                        } else {
