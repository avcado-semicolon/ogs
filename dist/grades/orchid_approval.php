...
if(isset($_POST['approved'])){
    ...
    $update = "UPDATE grading_system_2 SET status = 'approved' WHERE ID = '$id'";
    $result = mysqli_query($conn, $update);
        if($result == true){
            ...
            echo '}).then(function() {';
            echo '    window.location.href = "orchid_approval.php?approvalId=' . $personalId . '";';
            echo '});';
            echo '</script>';
        }else{
            echo '<script>';
...
if(isset($_POST['deny'])){
    ...
    $personalId = isset($_GET['approvalId']) ? $_GET['approvalId'] : null;
    $id = $_POST['ID'];
    $update = "UPDATE grading_system_2 SET status = 'denied' WHERE ID = '$id'";
    $result = mysqli_query($conn, $update);
...
<body class="bg-gray-200 font-sans">
                ...
                <table class="w-full border-collapse  mb-2">
                    ...
                    <?php 
                        ...
                        <tr> 
                            ...
                            <td class="py-2 px-4 border-2 border-slate-900 text-center">
                                <form action="orchid_approval.php?approvalId=<?php echo $rs['student_id']; ?>" method="POST" class="relative flex items-center">
            ...
            <div class="flex flex-col md:flex-row mt-4 ml-[325px] mb-4">
                <?php 
                     $personalId = isset($_GET['approvalId']) ? $_GET['approvalId'] : null;
                ?>
                <button class="bg-indigo-500 text-white mx-2 py-2 px-4 rounded-md hover:bg-indigo-700 mb-2 md:mb-0" onClick="window.location.replace('grades_2.php?viewGrades=<?php echo $personalId; ?>');">Back</button>
            </div>
